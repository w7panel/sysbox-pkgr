...
runtime_loaded() {
	binary=$1
	crictl --runtime-endpoint "unix://$containerd_socket" info 2>/dev/null |
...
if runtime_loaded "$bootstrap_runtime_binary"; then
	...
	while :; do
		if runtime_loaded "$bootstrap_runtime_binary" && \
			[ -S /run/sysbox/sysmgr.sock ] && \
			[ -S /run/sysbox/sysfs.sock ] && \
			[ -S /run/sysbox/sysbox-snapshotter.sock ]; then
